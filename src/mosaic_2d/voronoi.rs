...
use crate::{mosaic_2d::{delaunay::*, triangle_node2d::TriangleNode2d}, prelude::{is_point_within_edge_range_limt, is_vertex_within_polygon, sort_vertices_2d}};
			...
			for this_vert_id in this_tri.get_vertex_ids() {
				...
				for (other_tri_id, other_tri) in triangle_store.iter() {
					if this_tri_id != other_tri_id {
						if other_tri.get_vertex_ids().contains(this_vert_id) {
...
///
/// The grouping forms the key and the value is the Delaunay vertex ID they all have in common
fn find_shared_sets(map: &BTreeMap<usize, TriangleNode2d>) -> BTreeMap<Vec<usize>, usize> {
	let mut set = BTreeMap::new();
	for (id, triangle) in map {
		// compare each vert with the verts of all the other triangles
		let tri_verts_ids = triangle.get_vertex_ids();
		for vert_id in tri_verts_ids {
			// store the ID of each other_tri that shares this vertex
			let mut shared = vec![];
			for (other_id, other_tri) in map {
				if id != other_id {
					let other_abc = other_tri.get_vertex_ids();
					if other_abc.contains(&vert_id) {
			...
			if shared.len() >= 2 {
				let mut ids = shared;
				ids.push(*id);
				//TODO is sort needed? should sort by true Vec2?
				ids.sort();
				set.insert(ids, *vert_id);
			}
		}
	}
	set
...
fn triangulate_mesh(offset_cell_vertices: &Vec<Vec2>) -> Option<Mesh>{
			if let Some(delaunay) =
				Delaunay2d::compute_triangulation_2d(offset_cell_vertices)
			{
				let delaunay_triangles = delaunay.get_triangles();
				let delaunay_vertex_lookup = delaunay.get_vertex_lookup();

				// store all the vertices of the mesh
				let positions: Vec<Vec3> = offset_cell_vertices
					.iter()
					.map(|v| v.extend(0.0))
					.collect();
